...
export default function FormPanel({
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center px-4">
      ...
      <div
        className={`relative bg-[var(--bg-card)] border border-[var(--border-strong)] rounded-2xl p-6 w-full ${
      ...
      >
        ...
        <form onSubmit={onSubmit} className="space-y-3.5">
          ...
          <button
            type="submit"
            className="w-full bg-[var(--brown)] text-white font-medium rounded-lg py-2.5 text-sm hover:opacity-90 transition-opacity mt-2"
          >
            {submitLabel}
          </button>
        </form>
...
export const inputClass =
  "w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-lg py-2.5 px-3 text-sm outline-none focus:border-[var(--brown)] transition-colors";
          ...
          <thead>
            <tr className="border-b border-[var(--border-subtle)]">
              {columns.map((col, i) => (
                <th
                  key={i}
                  className={`text-left px-4 py-3 text-xs font-medium text-[var(--text-muted)] whitespace-nowrap ${col.className ?? ""}`}
                >
                  {col.header}
                </th>
              ))}
                ...
                {onDelete && (
                  <td className="px-2">
                    <button
                      ...
                      className="text-[var(--text-faint)] hover:text-[var(--red)] transition-colors p-1"
                    >
